...
| Trace width — 5V_RAIL, V_EXT_PROT, battery | 1.5mm | **2.0mm** |
| Isolation clearance | 0.3mm | **0.4mm** |
...
| SOT-23 pad — isolation gap | 0.25mm | **0.3mm** (fresh bits required) |
...
For 5V_RAIL at 500mA peak: use 2.0mm minimum. For 5V→encoder feeds (100mA each): 1.5mm.
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  ZONE A — POWER INPUT (top-left, ~55mm × 35mm)                           │
│                                                                           │
│  J_USB ──[D_USB]──┐                                                      │
...
│  MT3608 module ── [D_BOOST] ──► 5V_RAIL                                  │
│  C_BOOST (22µF), R_MT_HI (300kΩ), R_MT_LO (100kΩ) on module FB         │
│  C_LTC bypass (100nF), R_GATE (100kΩ), R_RPP (100kΩ)                   │
...
│  7× divider networks: R_TOP(10kΩ) + R_BOT(20kΩ) + C_FILT(10nF) + TVS  │
│  R_GPIO12 (10kΩ on GPIO12 divider node → GND)                            │
│                                                                           │
├──────────────────────────────────────────────────────────────────────────┤
...
- D_BAR and D_USB: adjacent to connectors, anodes toward connectors
- Q_RPP (SOT-23): between OR diodes and LTC4412; use fresh isolation bit
- LTC4412 (SOT-23-6): close to Q_SWITCH (AO3401); C_LTC bypass within 5mm of LTC4412 VIN pin
- Q_SWITCH (AO3401): drain toward 5V_RAIL bus; source toward V_EXT_PROT
- C1 (220µF): at 5V_RAIL star point; all ground spokes originate here
...
- MT3608 module: adjacent to TP4056; FB pin resistors soldered directly to module
- J5 (JST-PH): near MT3608 for short battery wires; short wires = less voltage drop
- D_BOOST: between MT3608 output and 5V_RAIL bus; anode to MT3608
...
- Divider networks: place each complete network (R_TOP, R_BOT, C_FILT, TVS) in a vertical column between its encoder connector and the 74HC14N
- R_GPIO12: on the Theta B divider node (before 74HC14N input), to GND
...
- 74HC14N VCC (pin14) to ESP32 3.3V pin; 100nF bypass between pin14 and pin7
...
3. Apply solder paste with the S63 integrated dispenser, or manually with a toothpick
4. Solder with a fine-tip iron (1.5mm chisel or 0.8mm conical) and 0.5mm solder wire
5. Check for bridges under magnification (10× loupe or macro camera) before powering
6. Three AO3401 pins: source (top pin), gate (left pin), drain (right pin) — verify from datasheet
...
**Solder first:**
1. D_BAR, D_USB (SS34 DO-201): test polarity with diode checker
2. Q_RPP (AO3401 SOT-23): inspect for bridges
3. LTC4412 (SOT-23-6): inspect for bridges
4. Q_SWITCH (AO3401 SOT-23): inspect for bridges
5. C_LTC (100nF), R_GATE (100kΩ), R_RPP (100kΩ)
6. C1 (220µF), C2 (100nF)
7. LED1 + R_LED1
8. TP4056 module (pin headers), MT3608 module (pin headers with fixed resistors already installed)
9. D_BOOST (SS34), C_BOOST (22µF)
10. R_MON1, R_MON2, C_ADC (battery monitor)
11. J5 (JST-PH LiPo connector)
12. J4 (barrel jack), J_USB (USB-C THT)
13. J6 (test screw terminal)

**Phase 1 Checkpoint:**
- Apply 5V at J4 → LED1 lights green
- TP1 = 4.98V (±0.1V) [LTC4412 active, ~20mV drop]
- TP5 = 0V (GND)
...
20. 7× divider networks: for each channel, solder R_TOP + R_BOT + C_FILT + TVS
21. R_GPIO12 (10kΩ pull-down on Theta B divider node → GND)
22. 74HC14N DIP-14 (check orientation notch; VCC=pin14 to ESP32 3.3V, 100nF bypass)
23. Wire 74HC14N outputs to correct ESP32 GPIO pins (see schematic Section 5)
